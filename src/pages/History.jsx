...
const History = () => {
  ...
  const requestDeleteSession = (id, e) => {
    ...
    setConfirmModal({
        ...
        message: "This will permanently remove this entire workout and all its data.",
        type: "danger",
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">
          <span className="font-bold text-gray-800 dark:text-white">{workouts.length}</span> Sessions
        </div>
      </div>
      ...
      {statsModalData && <ExerciseStatsModal exercise={statsModalData} onClose={() => setStatsModalData(null)} />}
...
// -----------------------------------------------------------------------------
const ConfirmationModal = ({ isOpen, title, message, type = 'danger', onClose, onConfirm }) => {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-4 animate-scale-in border border-gray-100 dark:border-gray-700 transition-colors">
                <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-full ${type === 'danger' ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400' : 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400'}`}>
                        {type === 'danger' ? <AlertCircle size={24} /> : <AlertTriangle size={24} />}
                    </div>
...
const EditExerciseModal = ({ data, onClose, onSuccess }) => {
  ...
  const isCardio = data.exercise?.bodyPart === 'Cardio' || data.exercise?.type === 'cardio';
  ...
  const addSet = () => {
    const lastWeight = formData.sets.length > 0 ? formData.sets[formData.sets.length-1].weight : '';
    setFormData({ ...formData, sets: [...formData.sets, { setNumber: formData.sets.length + 1, weight: lastWeight, reps: '' }] });
  };

  const removeSet = (index) => {
    const newSets = formData.sets.filter((_, i) => i !== index).map((s, i) => ({ ...s, setNumber: i + 1 }));
    setFormData({ ...formData, sets: newSets });
  };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh] transition-colors">
                <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-700/50">
                    <h3 className="font-bold text-gray-800 dark:text-white">Edit {data.exercise?.name}</h3>
                    <button onClick={requestClose}><X size={20} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300" /></button>
                </div>
                
                <div className="p-6 overflow-y-auto">
                    {isCardio ? (
                        <div className="grid grid-cols-2 gap-4">
                            {['Time (m)', 'Dist (km)', 'Speed', 'Calories'].map((label, i) => (
                                <div key={i} className="space-y-1">
                                    ...
                                    <input 
                                        ...
                                        onChange={e => setFormData({...formData, [Object.keys(formData)[i + 2]]: e.target.value})} 
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-10 gap-2 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase text-center">
                                <div className="col-span-1">#</div><div className="col-span-4">kg</div><div className="col-span-4">Reps</div><div className="col-span-1"></div>
                            </div>
                            {formData.sets.map((set, i) => (
                                <div key={i} className="grid grid-cols-10 gap-2 items-center">
                                    <div className="col-span-1 flex justify-center font-bold text-gray-500 dark:text-gray-400">{set.setNumber}</div>
                                    <div className="col-span-4"><input type="number" value={set.weight} onChange={(e) => updateSet(i, 'weight', e.target.value)} className="w-full p-2 border border-gray-200 dark:border-gray-600 rounded-lg text-center font-bold bg-white dark:bg-gray-700 text-gray-800 dark:text-white" /></div>
                                    <div className="col-span-4"><input type="number" value={set.reps} onChange={(e) => updateSet(i, 'reps', e.target.value)} className="w-full p-2 border border-gray-200 dark:border-gray-600 rounded-lg text-center font-bold bg-white dark:bg-gray-700 text-gray-800 dark:text-white" /></div>
                                    <div className="col-span-1 flex justify-center"><button onClick={() => removeSet(i)} className="text-red-400 dark:text-red-500 hover:text-red-600"><Trash2 size={16}/></button></div>
                                </div>
                            ))}
                            <button onClick={addSet} className="w-full py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl font-bold text-sm hover:bg-blue-100 dark:hover:bg-blue-900/40 flex items-center justify-center gap-1"><Plus size={16}/> Add Set</button>
                        </div>
                    )}
                </div>

                <div className="p-4 border-t border-gray-100 dark:border-gray-700 flex gap-3 bg-white dark:bg-gray-800">
                    <button onClick={requestClose} className="flex-1 py-3 font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl">Cancel</button>
                    <button onClick={handleSave} disabled={loading} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 flex items-center justify-center gap-2">
...
const ExerciseStatsModal = ({ exercise, onClose }) => {
  ...
  const [loading, setLoading] = useState(true);
  const isCardio = exercise.type === 'cardio' || (exercise.bodyPart && exercise.bodyPart === 'Cardio');
    ...
    const fetchHistory = async () => {
      try {
        ...
        const stats = allWorkouts.map(workout => {
            ...
            if (!log) return null;
            let val = isCardio ? (log.cardioDistance || log.cardioTime || 0) : (log.sets && log.sets.length > 0 ? Math.max(...log.sets.map(s => s.weight || 0)) : 0);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors">
        ...
        <div className="p-6 overflow-y-auto">
          {loading ? <div className="h-40 flex items-center justify-center"><Loader2 className="animate-spin text-blue-600" size={32} /></div> : history.length === 0 ? <div className="text-center py-10 text-gray-400"><TrendingUp className="mx-auto mb-3 opacity-20" size={48} /><p>No valid data points found.</p></div> : (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30"><div className="flex items-center gap-2 mb-1 text-blue-600 dark:text-blue-400 font-bold text-xs uppercase tracking-wider"><Trophy size={14} /> Personal Best</div><div className="text-2xl font-black text-gray-800 dark:text-white">{personalBest} <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{unit}</span></div></div>
                <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-100 dark:border-green-900/30"><div className="flex items-center gap-2 mb-1 text-green-600 dark:text-green-400 font-bold text-xs uppercase tracking-wider"><TrendingUp size={14} /> Latest</div><div className="text-2xl font-black text-gray-800 dark:text-white">{recentVal} <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{unit}</span></div></div>
              </div>
