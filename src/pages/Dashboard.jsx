...
const calculateVolume = (log) => {
  ...
  return log.exercises.reduce((acc, ex) => {
    if (ex.sets) {
      return (
        acc + ex.sets.reduce((sAcc, set) => sAcc + set.weight * set.reps, 0)
      );
    }
    return acc;
  }, 0);
...
const Dashboard = () => {
  ...
  const totalVolume = userLogs.reduce(
    (acc, log) => acc + calculateVolume(log),
    0,
  );
  ...
  const chartData = useMemo(() => {
    ...
    if (graphRange === "1W") {
      ...
      dataMap = days.map((day) => ({
        date: format(day, "EEE"),
        volume: getVolForDate(day),
      }));
      ...
      dataMap = months.map((monthStart) => {
        const logsInMonth = userLogs.filter((log) =>
          isSameMonth(parseISO(log.date), monthStart),
        );
        const totalVol = logsInMonth.reduce(
          (acc, log) => acc + calculateVolume(log),
          0,
        );
  ...
  const getWorkoutsForDay = (day) =>
    userLogs.filter((log) => isSameDay(parseISO(log.date), day));
              ...
              <div className='absolute inset-0 flex items-center justify-center bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm z-10 rounded-xl'>
                <div className='text-center'>
                  ...
                  <p className='text-xs text-gray-400 dark:text-gray-500'>
                    Log your first workout to see the graph!
                  </p>
                </div>
              </div>
            )}
          ...
          <div className='flex justify-between mb-6'>
            ...
            <div className='flex gap-2'>
              <button
                ...
                className='p-1 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 rounded'
              >
                <ChevronLeft size={20} />
              </button>
              <button
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    hasWorkout &&
                      "bg-blue-600 text-white shadow-md shadow-blue-200 dark:shadow-none cursor-pointer hover:scale-105",
                  )}
                >
                  {format(day, "d")}
                  {hasWorkout && (
...
const StatCard = ({ icon: Icon, color, bgColor, label, value }) => (
  <div className='bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-4 transition hover:-translate-y-1'>
    <div className={`p-4 ${bgColor} ${color} rounded-xl`}>
      <Icon size={24} />
    </div>
    <div>
