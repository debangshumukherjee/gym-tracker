      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-gray-500 dark:text-gray-400 text-sm'>
            Click any exercise to see your progress.
          </p>
        </div>
        <button
            ...
            return (
              ...
              >
                <div className='flex items-center gap-3'>
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${isCardio ? "bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 group-hover:bg-orange-100 dark:group-hover:bg-orange-900/40" : "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40"}`}
                  >
                  ...
                  <div>
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-md uppercase tracking-wide border ${isCardio ? "bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-900/30" : "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600"}`}
                    >
                      {ex.bodyPart}
                    </span>
                  </div>
                    ...
                    <>
                      ...
                      <div className='flex gap-1 pl-2 border-l border-gray-100 dark:border-gray-700'>
                        <div
                          ...
                          className='p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg cursor-pointer transition'
                        >
                          <Edit2 size={16} />
                        </div>
                        <div
                          onClick={(e) => requestDelete(ex.id, e)}
                          className='p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg cursor-pointer transition'
                        >
                          <Trash2 size={16} />
                        </div>
                      </div>
      ...
      {(showCreateModal || editExerciseData) && (
        <CreateExerciseModal
          ...
          onSuccess={(newEx) => {
            if (editExerciseData) {
              setExercises(
                exercises.map((e) => (e.id === newEx.id ? newEx : e)),
              );
            } else {
    ...
    <div className='fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in transition-all'>
      <div className='bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center transform transition-all scale-100 border border-gray-100 dark:border-gray-700'>
        ...
        <div className='grid grid-cols-2 gap-3'>
          <button
            onClick={onClose}
            className='py-3 px-4 rounded-xl font-bold text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition border border-gray-200 dark:border-gray-600'
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className='py-3 px-4 rounded-xl font-bold text-white bg-red-500 hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700 shadow-lg shadow-red-200 dark:shadow-none transition'
          >
            Yes, Delete
          </button>
        </div>
...
const CreateExerciseModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...formData,
      type: formData.bodyPart === "Cardio" ? "cardio" : "strength",
    };
    try {
      const token = localStorage.getItem("token");
      const url = isEditMode
        ? `${API_URL}/api/exercises/${initialData.id}`
      ...
      <div className='bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-fade-in border border-gray-100 dark:border-gray-700'>
        <div className='flex justify-between items-center mb-6'>
          ...
          <button onClick={onClose}>
            <X className='text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300' />
          </button>
        </div>
        <form onSubmit={handleSubmit} className='space-y-4'>
          <div>
            ...
            <input
              required
              autoFocus
              className='w-full p-3 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white dark:bg-gray-700 text-gray-800 dark:text-white'
              value={formData.name}
              onChange={(e) =>
          ...
          <div>
            ...
            <select
              className='w-full p-3 border border-gray-200 dark:border-gray-600 rounded-xl outline-none bg-white dark:bg-gray-700 text-gray-800 dark:text-white'
            ...
            if (isCardio) val = log.cardioDistance || log.cardioTime || 0;
              ...
              val =
                log.sets && log.sets.length > 0
                  ? Math.max(...log.sets.map((s) => s.weight))
                  : 0;
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div className='bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30'>
                  <div className='flex items-center gap-2 mb-1 text-blue-600 dark:text-blue-400 font-bold text-xs uppercase tracking-wider'>
                    <Trophy size={14} /> Personal Record
                  </div>
                ...
                <ResponsiveContainer width='100%' height='100%'>
                  <AreaChart data={history}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => [
                        `${value} ${unitLabel}`,
                        isCardio ? "Distance/Time" : "Max Weight",
                      ]}
                    />
                    <Area
                      type='monotone'
